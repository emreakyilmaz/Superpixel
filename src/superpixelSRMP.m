function [ww, imm, iwm, edges, alphaMat] = superpixelSRMP(fileName, S, alpha, numberOfIterations)
...
for n=1:numberOfIterations
    ...
    for r = 1:N1
        for c = 1:N2
            ...
            if(lbl(r,c) < 0)
                if(r-1 > 0 && r-1 > 0)
                    lbl(r,c) = lbl(r-1,c-1);  % Upper left.  r = row, c = column.
                elseif(r-1 > 0)
                   lbl(r,c) = lbl(r-1,c);    % Upper middle.  r = row, c = column.
                elseif(r-1 > 0 && c+1 < N2)
                    lbl(r,c) = lbl(r-1,c+1);  % Upper right.  r = row, c = column.
                elseif(c-1 > 0)
                    lbl(r,c) = lbl(r,c-1);    % left.  r = row, c = column.
                elseif(c+1 < N2)
                    lbl(r,c) = lbl(r,c+1);    % right. r = row, c = column.
                elseif(r+1 < N1 && c+1 < N2)
                    lbl(r,c) = lbl(r+1,c+1);  % Lowerleft.  r = row, c = column.
                elseif(r+1 < N1)
                    lbl(r,c) = lbl(r+1,c);    % lower middle.  r = row, c = column.
                elseif(r+1 < N1 && c-1 > 0)
                    lbl(r,c) = lbl(r+1,c-1);  % Lower left.  r = row, c = column.
                end
            end
